...
export default {
  ...
  methods:{
      ...
      this.chartInstance.on('click', async arg => {
        if (this.flag === 'china') {
          ...
          for (let i = 0;i<this.positionTemp.length;i++){
            if (this.positionTemp[i] === arg.name){
              charge = 1;
              break;
            }
          }
          if (charge === 0 ){
            this.positionTemp[this.positionTemp.length] = arg.name
            this.submit()
          }
    ...
    submit(){
      ...
      for (let i=0;i < this.positionTemp.length;i++){
        this.selForm.Position[this.selForm.Position.length + i] = this.positionTemp[i];
      }
      //处理开关数据
      if(this.Switch === true){
